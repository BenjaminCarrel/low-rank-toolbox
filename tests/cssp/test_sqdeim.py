...
def test_sqdeim_conditioning_bound(orthonormal_matrix):
    ...
    assert sigma_min_inv <= bound * 1.1, f"Conditioning bound violated: {sigma_min_inv} > {bound}"
...
def test_sqdeim_projector_computation(orthonormal_matrix):
    ...
    assert np.allclose(interpolated, test_vec, rtol=1e-9), "P_U interpolation property failed"
...
def test_sqdeim_complex_projector(complex_orthonormal_matrix):
    ...
    assert np.allclose(interpolated, test_vec, rtol=1e-9), "Complex P_U interpolation property failed"
...
def test_sqdeim_rank_one_projector(rank_one_matrix):
    ...
    assert np.allclose(test_vec, interpolated, rtol=1e-10), "Rank-1 interpolation failed"
...
def test_sqdeim_vs_qdeim_both_work():
    ...
    assert la.svdvals(U_p_qdeim)[-1] > 1e-14, "QDEIM submatrix should be well-conditioned"
    assert la.svdvals(U_p_sqdeim)[-1] > 1e-14, "sQDEIM submatrix should be well-conditioned"
