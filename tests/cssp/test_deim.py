...
def test_deim_complex_return_inverse(complex_orthonormal_matrix):
    ...
    assert np.allclose(inv_U, expected_inv_U, rtol=1e-10), "Complex inv_U != inv(U[p, :])"
...
def test_deim_preserves_span(orthonormal_matrix):
    ...
    assert np.allclose(test_vec, reconstructed, rtol=1e-10), "Interpolation should preserve span"
...
def test_deim_with_solve_kwargs(orthonormal_matrix):
    ...
    assert np.allclose(P_U1, P_U2, rtol=1e-10), "Empty solve_kwargs shouldn't change projector"
...
def test_deim_matrix_inversion_accuracy(orthonormal_matrix):
    ...
    assert np.allclose(identity_check, eye_k, rtol=1e-10), "inv_U @ U[p,:] should be identity"
...
def test_deim_indices_differ_from_first_k():
    ...
    assert p != list(range(5)), "DEIM should select indices based on greedy algorithm, not sequential"
