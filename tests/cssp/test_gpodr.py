...
def test_gpodr_with_tolerance(orthonormal_matrix):
    ...
    assert (
        sigma_min_inv <= tol * 1.1
    ), f"Tolerance condition violated: {sigma_min_inv} > {tol}"
...
def test_gpodr_starts_with_qdeim(orthonormal_matrix):
    ...
    assert (
        qdeim_set == gpodr_set
    ), "gpodr with oversampling=0 should match QDEIM indices"
...
def test_gpodr_lstsq_accuracy(orthonormal_matrix):
    ...
    ), "P_U doesn't satisfy least-squares property"
...
def test_gpodr_vs_gpode_both_work():
    ...
    assert (
        len(p_gpodr) == 5 + oversampling
    ), "gpodr should return k+oversampling indices"
    assert (
        len(p_gpode) == 5 + oversampling
    ), "gpode should return k+oversampling indices"
    ...
    assert (
        la.svdvals(U_p_gpodr)[-1] > 1e-14
    ), "gpodr submatrix should be well-conditioned"
    assert (
        la.svdvals(U_p_gpode)[-1] > 1e-14
    ), "gpode submatrix should be well-conditioned"
