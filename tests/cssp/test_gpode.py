...
def test_gpode_return_both(orthonormal_matrix):
    ...
    p, M, inv_U = gpode(Q, oversampling_size=oversampling, return_projector=True, return_inverse=True)
...
def test_gpode_starts_with_qdeim(orthonormal_matrix):
    ...
    assert qdeim_set == gpode_first_k_set, "First k gpode indices should match QDEIM indices"
...
def test_gpode_pseudoinverse_accuracy(orthonormal_matrix):
    ...
    assert np.allclose(reconstructed_UT, expected_UT, rtol=1e-9), "M doesn't satisfy least-squares property"
...
def test_gpode_with_lstsq_kwargs(orthonormal_matrix):
    ...
    p1, M1 = gpode(Q, oversampling_size=oversampling, return_projector=True, lstsq_kwargs={'cond': None})
    p2, M2 = gpode(Q, oversampling_size=oversampling, return_projector=True)
    
    # Results should be very similar
    assert np.allclose(M1, M2, rtol=1e-9), "lstsq_kwargs shouldn't significantly change result"
...
def test_gpode_greedy_improvement(orthonormal_matrix):
    ...
    for i in range(k, len(p)):
        ...
        assert sigma_after[-1] > 1e-14, f"Singular value too small after adding index {i}"
...
def test_gpode_documentation_example():
    ...
    assert M.shape == (50, 8 + oversampling), "Projector should be n x m (n x (k+oversampling))"
    
    # Verify least-squares interpolation property
    test_vector = U @ np.random.randn(8)
    interpolated = M @ test_vector[p]
    assert np.allclose(test_vector, interpolated, rtol=1e-9), "Should interpolate accurately with oversampling"
    
    # With both outputs
    p, M, inv_U = gpode(U, oversampling_size=oversampling, return_projector=True, return_inverse=True)
