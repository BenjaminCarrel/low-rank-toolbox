...
def test_givens_unitary_parametrized(x, y):
    ...
    assert np.allclose(
        identity, expected_identity, atol=1e-10
    ), f"Failed for x={x}, y={y}"
...
def test_givens_large_values(large_real_vector):
    ...
    expected_r = np.hypot(x, y)
    assert (
        np.abs(result[0] - expected_r) / expected_r < 1e-10
...
def test_givens_vastly_different_magnitudes():
    ...
    assert (
        np.abs(result[0] - expected_r) / expected_r < 1e-10
    ), "Should handle magnitude difference"
    assert np.abs(result[1]) < 1e-5, "Second component should be near zero"
...
def test_givens_result_is_real_nonnegative():
    ...
    for _ in range(10):
        ...
        assert (
            np.abs(np.imag(result[0])) < 1e-10
        ), f"First component should be real for x={x}, y={y}"
        assert (
            result[0].real >= -1e-10
        ), f"First component should be non-negative for x={x}, y={y}"
...
def test_givens_inverse_is_conjugate_transpose():
    ...
    assert np.allclose(
        G_inv, G_H, atol=1e-12
    ), "Inverse should equal conjugate transpose"
...
def test_givens_double_application():
    ...
    G_combined = G1 @ G2
    assert np.allclose(
        G_combined @ G_combined.conj().T, np.eye(2, dtype=np.complex128), atol=1e-12
    )
...
def test_givens_real_matches_standard():
    ...
    G_expected = np.array(
        [[c_expected, s_expected], [-s_expected, c_expected]], dtype=np.complex128
    )
...
def test_givens_random_complex(seed):
    ...
    expected_r = np.hypot(np.abs(x), np.abs(y))
    assert (
        np.abs(result[0] - expected_r) < 1e-10
    ), f"Seed {seed}: first component incorrect"
