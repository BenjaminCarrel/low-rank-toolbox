...
def test_sRRQR_rank_complex(complex_low_rank_matrix):
    ...
    assert np.allclose(
        QtQ, eye_k, rtol=1e-10
    ), "Q should be unitary for complex matrices"
...
def test_sRRQR_rank_complex_triangular(complex_low_rank_matrix):
    ...
    assert np.allclose(
        R_lower, 0, atol=1e-12
    ), "R should be upper triangular for complex"
...
def test_sRRQR_complex_eta_bound(complex_low_rank_matrix):
    ...
    assert (
        max_entry <= eta + 1e-10
    ), f"Eta bound violated for complex: {max_entry} > {eta}"
...
def test_sRRQR_tol_eta_less_than_one():
    ...
    if k > 0:
        QtQ = Q.T @ Q
        assert np.allclose(
            QtQ, np.eye(k), rtol=1e-10
        ), "Valid factorization despite eta<1"
...
def test_sRRQR_rank_reconstruction_accuracy(low_rank_matrix):
    ...
    error = np.linalg.norm(A_permuted[:, :k] - A_approx, "fro") / np.linalg.norm(
        A, "fro"
    )
...
def test_sRRQR_full_reconstruction():
    ...
    error = np.linalg.norm(A_permuted - A_reconstructed, "fro") / np.linalg.norm(
        A, "fro"
    )
...
def test_sRRQR_rank_diagonal_positive():
    ...
    assert np.all(
        np.real(diag_R) >= -1e-14
    ), "Diagonal should have non-negative real part"
...
def test_sRRQR_vs_standard_qr():
    ...
    # Check orthogonality
    assert np.allclose(
        Q_srrqr.T @ Q_srrqr, np.eye(k), rtol=1e-10
    ), "sRRQR Q orthonormal"
