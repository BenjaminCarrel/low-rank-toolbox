...
def generate_lyapunov_problem(n, rank, is_symmetric=True, seed=42):
    ...
    if is_symmetric:
        X = SVD.generate_random(
            (n, n), np.logspace(-1, -rank, rank), seed=seed, is_symmetric=True
        )
    else:
        X = SVD.generate_random(
            (n, n), np.logspace(-1, -rank, rank), seed=seed, is_symmetric=False
        )
...
class TestKrylovMethods:
    ...
    def test_classic_krylov(self):
        ...
        with pytest.warns(UserWarning, match="standard Krylov space may not converge"):
            X = solve_lyapunov(
                A, C, is_symmetric=True, krylov_kwargs=krylov_kwargs, max_iter=50
            )
...
class TestToleranceCriterion:
    ...
    def test_different_tolerances_nonsymmetric(self):
        ...
        for tol in [1e-6, 1e-8, 1e-10]:
            ...
            assert (
                residual <= tol * 10
            ), f"Residual {residual} not within tolerance {tol}"
...
class TestQuickValidation:
    ...
    def test_small_symmetric_problem(self):
        ...
        C = SVD.generate_random(
            (n, n), np.array([1.0, 0.5, 0.1]), is_symmetric=True, seed=42
        )
    ...
    def test_small_nonsymmetric_problem(self):
        ...
        A = sps.eye(n, format="csc") * 2.0
        C = SVD.generate_random(
            (n, n), np.array([1.0, 0.5, 0.1]), is_symmetric=False, seed=43
        )
    ...
    def test_1d_laplacian_50x50(self):
        ...
        A = create_1d_laplacian(n)
        C = SVD.generate_random(
            (n, n), np.logspace(-1, -10, 10), is_symmetric=True, seed=44
        )
    ...
    def test_krylov_methods_comparison(self):
        ...
        X1 = solve_lyapunov(
            A, C, is_symmetric=True, krylov_kwargs={"extended": True}, tol=1e-10
        )
...
class TestLanczosUsage:
    ...
    def test_lanczos_used_for_symmetric_krylov_space(self):
        ...
        assert not hasattr(
            ks, "H"
        ), "Arnoldi H matrix should not exist for symmetric case"
    ...
    def test_arnoldi_used_for_nonsymmetric_krylov_space(self):
        ...
        assert not hasattr(
            ks, "_alpha"
        ), "Lanczos alpha should not exist for non-symmetric case"
        assert not hasattr(
            ks, "_beta"
        ), "Lanczos beta should not exist for non-symmetric case"
    ...
    def test_lanczos_in_extended_krylov_space(self):
        ...
        assert hasattr(
            eks.krylov_space, "_alpha"
        ), "Krylov component doesn't use Lanczos"
        assert hasattr(
            eks.krylov_space, "_beta"
        ), "Krylov component doesn't use Lanczos"
        assert hasattr(
            eks.inverted_krylov_space, "_alpha"
        ), "Inverted Krylov component doesn't use Lanczos"
        assert hasattr(
            eks.inverted_krylov_space, "_beta"
        ), "Inverted Krylov component doesn't use Lanczos"
    ...
    def test_rational_krylov_no_lanczos(self):
        ...
        assert not hasattr(
            rks, "_alpha"
        ), "RationalKrylovSpace doesn't use Lanczos (known limitation)"
    ...
    def test_lanczos_efficiency_symmetric_vs_nonsymmetric(self):
        ...
        lanczos_params = sum(
            x.size if x is not None and hasattr(x, "size") else 0
            for x in [
                ks_sym._alpha[i]
                for i in range(ks_sym.m)
                if ks_sym._alpha[i] is not None
            ]
        )
    ...
    def test_symmetry_flag_propagation(self):
        ...
        with warnings.catch_warnings():
            warnings.filterwarnings(
                "ignore", message=".*standard Krylov space may not converge.*"
            )
            X = solve_sparse_low_rank_symmetric_lyapunov(
                A, C, tol=1e-10, krylov_kwargs={"extended": False}
            )
