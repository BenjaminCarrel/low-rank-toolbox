...
from low_rank_toolbox.krylov.spaces.extended_krylov_space import ExtendedKrylovSpace
...
def test_vector_KrylovSpace():
    ...
    KS.augment_basis()
    assert np.allclose(
        la.norm(KS.Q, axis=0), np.ones(2 * m)
    ), "The columns of Q are not normalized -> error in augment basis"
    assert (
        la.norm(KS.Q.T.dot(KS.Q) - np.eye(2 * m)) < 1e-10
    ...
    KS.augment_basis()
    assert np.allclose(
        la.norm(KS.Q, axis=0), np.ones(3 * 2)
    ), "The columns of Q are not normalized -> error in augment basis"
    assert (
        la.norm(KS.Q.T.dot(KS.Q) - np.eye(3 * 2)) < 1e-10
    ), "The columns of Q are not orthogonal -> error in augment basis"
    assert (
        la.norm(KS.Q.dot(KS.Q.T) - Q_ref.dot(Q_ref.T)) < 1e-10
    ), "Wrong projection -> error in augment basis"
...
def test_block_KrylovSpace():
    ...
    # Check the reference
    assert np.allclose(
        la.norm(Q_ref, axis=0), np.ones(2 * m * r)
    ), "The columns of Q are not normalized -> error in the QR decomposition"
    assert (
        la.norm(Q_ref.T.dot(Q_ref) - np.eye(2 * m * r)) < 1e-10
    ...
    assert KS.Q.shape == (
        20,
        2 * m * r,
    ), "Wrong shape of Q -> error in block Krylov Space"
    assert KS.Am.shape == (
        2 * m * r,
        2 * m * r,
    ), "Wrong shape of Am -> error in block Krylov Space"
    assert np.allclose(
        la.norm(KS.Q, axis=0), np.ones(2 * m * r)
    ), "The columns of Q are not normalized -> error in block Krylov Space"
    assert (
        la.norm(KS.Q.T.dot(KS.Q) - np.eye(2 * m * r)) < 1e-10
...
def test_property_H1_H2():
    ...
    # Check dimensions
    assert (
        H1.shape[0] == H1.shape[1] + 1 or H1.shape[0] == H1.shape[1]
    ), "H1 wrong shape"
    assert (
        H2.shape[0] == H2.shape[1] + 1 or H2.shape[0] == H2.shape[1]
    ), "H2 wrong shape"
...
def test_extended_krylov_with_custom_invA():
    ...
    Q = EKS.Q
    assert np.allclose(
        Q.T @ Q, np.eye(Q.shape[1]), atol=1e-8
    ), "Q not orthonormal with custom invA"
...
def test_extended_krylov_symmetric_flag():
    ...
    assert (
        EKS.inverted_krylov_space.is_symmetric
    ), "Inverted Krylov space should detect symmetry"

    Q = EKS.Q
    assert np.allclose(
        Q.T @ Q, np.eye(Q.shape[1]), atol=1e-10
    ), "Symmetric case: Q not orthonormal"
...
def test_extended_krylov_multiple_augmentations():
    ...
    for i in range(len(sizes) - 1):
        assert (
            sizes[i + 1] > sizes[i]
        ), f"Size should increase: {sizes[i]} -> {sizes[i+1]}"

    # Final Q should be orthonormal
    Q = EKS.Q
    assert np.allclose(
        Q.T @ Q, np.eye(Q.shape[1]), atol=1e-9
    ), "Q not orthonormal after multiple augmentations"
...
def test_extended_krylov_block_case_properties():
    ...
    assert (
        Q1.shape[1] == 6
    ), f"Q1 should have 6 columns (2 blocks × 3), got {Q1.shape[1]}"
    assert (
        Q2.shape[1] == 6
    ), f"Q2 should have 6 columns (2 blocks × 3), got {Q2.shape[1]}"

    # Combined Q should be orthonormal
    assert np.allclose(
        Q.T @ Q, np.eye(Q.shape[1]), atol=1e-10
    ), "Block case: Q not orthonormal"
...
class TestExtendedKrylovProperties:
    ...
    def test_extended_krylov_caching(
        self, valid_extended_matrix, valid_extended_vector
    ):
...
class TestExtendedKrylovAdvancedFunctionality:
    ...
    def test_extended_krylov_combines_spaces(
        self, valid_extended_matrix, valid_extended_vector
    ):
