...
class TestInvertedKrylovSpecific:
    ...
    def test_custom_invA(self, valid_inverted_matrix, valid_inverted_vector):
        ...
        IK = InvertedKrylovSpace(valid_inverted_matrix, valid_inverted_vector, invA=custom_inv)
...
class TestInvertedKrylovAdvancedFunctionality:
    ...
    def test_inverted_krylov_span_consistency(self, valid_inverted_matrix, valid_inverted_vector):
        ...
        x_inv = A_inv(valid_inverted_vector.ravel()).reshape(valid_inverted_vector.shape)
        space = [x_inv]
        for _ in range(2):
        ...
        assert np.allclose(P_computed, P_ref, atol=1e-8), \
