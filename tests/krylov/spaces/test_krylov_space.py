...
class TestKrylovDimensionOverflow:
    ...
    def test_krylov_dimension_warning(self, valid_krylov_matrix, valid_krylov_vector):
        ...
        with pytest.warns(
            UserWarning, match="The next basis would exceed the dimension"
        ):
            KS.augment_basis()
...
class TestKrylovComputeAll:
    ...
    def test_krylov_compute_all_with_max_iter(
        self, valid_krylov_matrix, valid_krylov_vector
    ):
...
def test_space_structure_dtype():
    ...
    assert np.iscomplexobj(
        np.zeros(1, dtype=KS_mixed.dtype)
