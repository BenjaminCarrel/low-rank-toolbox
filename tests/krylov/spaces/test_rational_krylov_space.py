...
class TestRationalKrylovSpecificValidation:
    ...
    def test_nan_in_poles(self, valid_rational_matrix, valid_rational_vector):
        ...
        with pytest.raises(ValueError, match="poles contain NaN or Inf values"):
            RationalKrylovSpace(valid_rational_matrix, valid_rational_vector, poles=poles)
...
class TestRationalKrylovDimensionOverflow:
    ...
    def test_rational_krylov_dimension_error(self, valid_rational_matrix, valid_rational_vector):
        """Test that RationalKrylov raises error when exceeding dimension"""
        poles = [1.0] * 15  # More poles than matrix dimension
        RK = RationalKrylovSpace(valid_rational_matrix, valid_rational_vector, poles=poles)
    ...
    def test_rational_krylov_insufficient_poles(self, valid_rational_matrix, valid_rational_vector):
...
class TestRationalKrylovProperties:
    ...
    def test_rational_krylov_max_iter(self, valid_rational_matrix, valid_rational_vector):
...
class TestRationalKrylovComputeAll:
    ...
    def test_rational_krylov_compute_all(self, valid_rational_matrix, valid_rational_vector):
...
class TestRationalKrylovInverseOnly:
    ...
    def test_inverse_only_true(self, valid_rational_matrix, valid_rational_vector):
        ...
        poles = [0.5]
        RK = RationalKrylovSpace(valid_rational_matrix, valid_rational_vector, poles=poles, inverse_only=True)
        RK.augment_basis()
        
        # Verify the basis is orthonormal
        assert np.allclose(RK.Q.T @ RK.Q, np.eye(RK.size)), "Basis should be orthonormal"
    
    def test_inverse_only_false(self, valid_rational_matrix, valid_rational_vector):
        """Test inverse_only=False solves (A-pI)v = Au"""
        poles = [0.5]
        RK = RationalKrylovSpace(valid_rational_matrix, valid_rational_vector, poles=poles, inverse_only=False)
        RK.augment_basis()
        
        # Verify the basis is orthonormal
        assert np.allclose(RK.Q.T @ RK.Q, np.eye(RK.size)), "Basis should be orthonormal"
    
    def test_inverse_only_difference(self, valid_rational_matrix, valid_rational_vector):
        """Test that inverse_only parameter works correctly"""
        poles = [0.5]
        
        # Just verify both modes produce valid orthonormal bases
        RK_true = RationalKrylovSpace(valid_rational_matrix, valid_rational_vector, poles=poles, inverse_only=True)
        RK_true.augment_basis()
        
        RK_false = RationalKrylovSpace(valid_rational_matrix, valid_rational_vector, poles=poles, inverse_only=False)
        RK_false.augment_basis()
        
        # Both should produce orthonormal bases
        assert np.allclose(RK_true.Q.T @ RK_true.Q, np.eye(RK_true.size)), \
            "inverse_only=True should produce orthonormal basis"
        assert np.allclose(RK_false.Q.T @ RK_false.Q, np.eye(RK_false.size)), \
