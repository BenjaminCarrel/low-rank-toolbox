...
def test_Arnoldi():
    ...
    assert np.allclose(
        la.norm(Q_ref, axis=0), np.ones(m)
    ), "The columns of Q are not normalized -> error in the QR decomposition"
    assert (
        la.norm(Q_ref.T.dot(Q_ref) - np.eye(m)) < 1e-8
    ...
    assert np.allclose(
        la.norm(Q, axis=0), np.ones(m)
    ), "The columns of Q are not normalized -> error in Arnoldi"
    assert (
        la.norm(Q.T.dot(Q) - np.eye(m)) < 1e-8
    ), "The columns of Q are not orthogonal -> error in Arnoldi"
    assert H.shape == (m, m), "Wrong shape of H -> error in Arnoldi"
    print("Arnoldi Basis OK.")

    print(
        "Arnoldi - Error in the projection:", la.norm(Q.dot(Q.T) - Q_ref.dot(Q_ref.T))
    )
    assert (
        la.norm(Q.dot(Q.T) - Q_ref.dot(Q_ref.T)) < 1e-8
    ), "Wrong projection -> error in Arnoldi"
...
def test_shift_invert_Arnoldi():
    ...
    print(
        "Shift and invert Arnoldi - Error in the projection:",
        la.norm(Q.dot(Q.T) - Q_ref.dot(Q_ref.T)),
    )
    assert (
        la.norm(Q.dot(Q.T) - Q_ref.dot(Q_ref.T)) < 1e-8
    ), "Wrong projection -> error in Arnoldi"
...
def test_rational_Arnoldi():
    ...
    print(
        "Rational Arnoldi - Error in the projection:",
        la.norm(Q.dot(Q.T) - Q_ref.dot(Q_ref.T)),
    )
    assert (
        la.norm(Q.dot(Q.T) - Q_ref.dot(Q_ref.T)) < 1e-8
...
def test_block_Arnoldi():
    ...
    assert (
        la.norm(Q_ref.T.dot(Q_ref) - np.eye(m * r)) < 1e-8
    ), "The columns of Q are not orthogonal -> error in the QR decomposition"
    print("Reference OK.")
    ...
    assert np.allclose(
        la.norm(Q, axis=0), np.ones(m * r)
    ), "The columns of Q are not normalized -> error in block Arnoldi"
    assert (
        la.norm(Q.T.dot(Q) - np.eye(m * r)) < 1e-8
    ), "The columns of Q are not orthogonal -> error in block Arnoldi"
    print("Block Arnoldi Basis OK.")

    # Compare to the reference
    print(
        "Block Arnoldi - Error in the projection:",
        la.norm(Q.dot(Q.T) - Q_ref.dot(Q_ref.T)),
    )
    assert (
        la.norm(Q.dot(Q.T) - Q_ref.dot(Q_ref.T)) < 1e-8
    ), "Wrong projection -> error in block Arnoldi"
...
def test_block_shift_invert_Arnoldi():
    ...
    # Reference computation
    space = [
        spsla.spsolve(((A - s * sps.eye(n, format="csc")) ** i).tocsc(), X0)
        for i in range(m)
    ]
    Q_ref, R = la.qr(np.column_stack(space), mode="economic")
    ...
    assert (
        la.norm(Q.T.dot(Q) - np.eye(m * r)) < 1e-6
    ), "The columns of Q are not orthogonal -> error in block Arnoldi"
    print("Block shift and invert Arnoldi Basis OK.")

    # Compare to the reference
    print(
        "Block shift and invert Arnoldi - Error in the projection:",
        la.norm(Q.dot(Q.T) - Q_ref.dot(Q_ref.T)),
    )
    assert (
        la.norm(Q.dot(Q.T) - Q_ref.dot(Q_ref.T)) < 1e-4
    ), "Wrong projection -> error in block Arnoldi"
...
def test_block_rational_Arnoldi_shift_only():
    ...
    assert H.shape == (
        m * r,
        m * r,
    ), "Wrong shape of H -> error in block rational Arnoldi"
    assert np.allclose(
        la.norm(Q, axis=0), np.ones(m * r)
    ), "The columns of Q are not normalized -> error in block rational Arnoldi"
    assert (
        la.norm(Q.T.dot(Q) - np.eye(m * r)) < 1e-8
    ), "The columns of Q are not orthogonal -> error in block Arnoldi"
    ...
    # Compare to the reference
    print(
        "Block rational Arnoldi (inverse only) - Error in the projection:",
        la.norm(Q.dot(Q.T) - Q_ref.dot(Q_ref.T)),
    )
    assert (
        la.norm(Q.dot(Q.T) - Q_ref.dot(Q_ref.T)) < 1e-8
...
def test_block_rational_Arnoldi_full():
    ...
    # Compare to the reference
    print(
        "Block rational Arnoldi (full) - Error in the projection:",
        la.norm(Q.dot(Q.T) - Q_ref.dot(Q_ref.T)),
    )
    assert (
        la.norm(Q.dot(Q.T) - Q_ref.dot(Q_ref.T)) < 1e-5
    ), "Wrong projection -> error in block Arnoldi"
    print("Block rational Arnoldi Projection OK.")
...
def test_rational_Arnoldi_different_poles():
    ...
    assert np.allclose(
        np.abs(Q[:, 1].T.dot(Q_ref[:, 1])), 1.0, atol=1e-6
...
def test_block_rational_Arnoldi_different_poles():
    ...
    assert np.allclose(
        la.norm(Q, axis=0), np.ones(m * r), atol=1e-6
