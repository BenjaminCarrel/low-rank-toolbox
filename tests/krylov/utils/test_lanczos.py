...
def test_Lanczos():
    ...
    assert (
        la.norm(Q_ref.T.dot(Q_ref) - np.eye(m)) < 1e-10
    ...
    ), "The columns of Q are not normalized -> error in Lanczos"
    assert (
        la.norm(Q.T.dot(Q) - np.eye(m)) < 1e-10
    ), "The columns of Q are not orthogonal -> error in Lanczos"
    ...
    assert (
        la.norm(Q.dot(Q.T) - Q_ref.dot(Q_ref.T)) < 1e-10
    ), "Basis is not the same -> error in Lanczos"
...
def test_block_Lanczos():
    ...
    ), "The columns of Q are not normalized -> error in block Lanczos"
    assert (
        la.norm(Q.T.dot(Q) - np.eye(m * r)) < 1e-10
    ), "The columns of Q are not orthogonal -> error in block Lanczos"
    ...
    assert (
        la.norm(Q.dot(Q.T) - Q_ref.dot(Q_ref.T)) < 1e-10
    ), "Wrong projection -> error in block Lanczos"
...
def test_Lanczos_complex():
    ...
    assert (
        sps.linalg.norm(
            T
            - sps.diags(
                [T.diagonal(), T.diagonal(1), T.diagonal(-1)], [0, 1, -1], format="csc"
            )
        )
        < 1e-10
    ), "T not tridiagonal"
