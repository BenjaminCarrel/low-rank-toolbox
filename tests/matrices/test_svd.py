...
def test_SVD_class_methods(svd_basic):
    ...
    assert isinstance(SVD.generate_random((30, 24), np.asarray([4, 3, 2, 1])), SVD), "Incorrect type of generate_random"
...
def test_SVD_addition(svd_basic):
    ...
    if AUTOMATIC_TRUNCATION:
        assert (X + X).rank == X.rank, "Incorrect rank of addition with SVD (with auto-truncation)"
        assert (X - X).rank == 0, "Incorrect rank of subtraction with SVD (with auto-truncation)"
    else:
        assert (X + X).rank == 2*X.rank, "Incorrect rank of addition with SVD (with auto-truncation disabled)"
        assert (X-X).rank == 2*X.rank, "Incorrect rank of subtraction with SVD (with auto-truncation disabled)"
    
    assert np.allclose((X - X).full(), 0 * X_full), "Incorrect subtraction with SVD"
    Y = SVD.generate_random((30, 24), np.asarray([4, 3, 2, 1])) # Try with a different rank
...
def test_SVD_multiplication(svd_basic):
    ...
    Y = SVD.generate_random((24, 28), np.asarray([4, 3, 2, 1])) # Try with a different rank
...
def test_SVD_hadamard():
    ...
    if AUTOMATIC_TRUNCATION:
        assert Y.hadamard(Y).rank <= 2*rank, "Incorrect rank of Hadamard product with SVD (with auto-truncation)"
    else:
...
def test_SVD_init_complex_matrices():
    ...
    np.random.seed(42)
    U_complex, _ = la.qr(np.random.randn(10, 5) + 1j * np.random.randn(10, 5), mode='economic')
    V_complex, _ = la.qr(np.random.randn(8, 5) + 1j * np.random.randn(8, 5), mode='economic')
...
def test_SVD_transpose():
    ...
    assert np.allclose(X_T.T.full(), X.full()), "Double transpose should return original"
...
def test_SVD_sing_vals_property():
    ...
    assert np.array_equal(X.sing_vals(), X.s), "sing_vals() should return same values as s"
...
def test_SVD_compute_storage_size():
    ...
    full_size = m * n
    assert actual_size < full_size, "SVD should use less storage than full matrix for low rank"
...
def test_SVD_truncated_svd_rank():
    ...
    assert not np.allclose(X.full(), A), "Rank-5 approximation should differ from full matrix"
...
def test_SVD_truncated_svd_rtol():
    ...
    assert np.all(X_trunc.s > 100 * 0.02), "All singular values should be above threshold"
...
def test_SVD_scalar_multiplication():
    ...
    W = X * (-1.5)
    assert np.allclose(W.full(), X.full() * (-1.5)), "Negative scalar multiplication incorrect"
...
def test_SVD_consistency_after_operations():
    ...
    assert np.all(Z1.s[:-1] >= Z1.s[1:]), "Singular values should be sorted after addition"
    
    # Multiplication
    Z2 = X.dot(X.T)
    assert np.all(Z2.s >= 0), "Singular values should be non-negative after multiplication"
    assert np.all(Z2.s[:-1] >= Z2.s[1:]), "Singular values should be sorted after multiplication"
...
def test_SVD_size_property():
    ...
    assert X.size == expected_storage, f"Expected storage size {expected_storage}, got {X.size}"
...
def test_SVD_generate_random_seed():
    ...
    assert not np.allclose(X1.full(), X3.full()), "Different seed should produce different matrix"
...
def test_SVD_numerical_stability():
    ...
    assert np.allclose(X_reconstructed, X.full(), rtol=1e-5), "Should handle ill-conditioned matrices"
...
def test_SVD_Ut_Uh_Vt_Vh_properties():
    ...
    assert np.allclose(X_complex.Uh, X_complex.U.T.conj()), "Uh should be conjugate transpose"
    assert np.allclose(X_complex.Vt, X_complex.V.T), "Vt should be transpose (no conj)"
    assert np.allclose(X_complex.Vh, X_complex.V.T.conj()), "Vh should be conjugate transpose"
...
def test_SVD_S_property_2D():
    ...
    assert np.allclose(np.diag(X.S), sing_vals), "S diagonal should match singular values"
...
def test_SVD_compress():
    ...
    assert np.allclose(X_compressed.full(), X.full()), "Compression should preserve matrix"
...
def test_SVD_reconstruction_accuracy():
    ...
    # Compare with full()
    assert np.allclose(reconstructed, X.full()), "Manual reconstruction should match full()"
...
def test_SVD_transpose_property_preservation():
    ...
    assert np.isclose(X_T.norm('fro'), X.norm('fro')), "Transpose should preserve Frobenius norm"
...
def test_SVD_pseudoinverse_full_rank():
    ...
    assert np.allclose(X_pinv.full(), reconstruction_pinv.full()), "X⁺ @ X @ X⁺ should equal X⁺"
...
def test_SVD_pseudoinverse_rank_deficient():
    ...
    assert np.allclose(X_pinv.s, expected_s_pinv, atol=1e-10), "Small singular values should be zeroed"
...
def test_SVD_lstsq_multiple_rhs():
    ...
    for i in range(5):
        ...
        assert np.abs(residual - residual_numpy) < 1e-10, f"Column {i} residual differs from numpy"
...
def test_SVD_sqrtm_symmetric():
    ...
    assert np.allclose(X.full(), reconstruction.full()), "X^{1/2} @ X^{1/2} should equal X"
...
def test_SVD_expm_trace_property():
    ...
    assert np.allclose(actual_trace, expected_trace), "Trace of exp(X) should match sum of exp(eigenvalues)"
...
def test_SVD_solve_vs_lstsq_consistency():
    ...
    assert np.allclose(x_solve, x_lstsq), "solve and lstsq should agree for full-rank system"
...
def test_SVD_complex_pseudoinverse():
    ...
    reconstruction = X @ X_pinv @ X
    assert np.allclose(X.full(), reconstruction.full()), "Complex pseudoinverse reconstruction failed"
    
    # Test that X @ X⁺ is Hermitian
    XX_pinv = (X @ X_pinv).full()
    assert np.allclose(XX_pinv, XX_pinv.T.conj()), "X @ X⁺ should be Hermitian for complex"
...
def test_SVD_complex_lstsq():
    ...
    assert np.abs(residual_ours - residual_numpy) < 1e-10, "Complex lstsq doesn't match numpy"
...
def test_SVD_complex_sqrtm():
    ...
    reconstruction = X_sqrt @ X_sqrt
    assert np.allclose(X.full(), reconstruction.full(), atol=1e-10), "Complex sqrtm failed"
...
def test_SVD_complex_properties_cached():
    ...
    assert np.allclose(norm1, np.linalg.norm(A, 'fro')), "Complex Frobenius norm incorrect"
