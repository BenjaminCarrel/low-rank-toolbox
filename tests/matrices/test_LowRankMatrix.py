...
def test_LowRankMatrix_addition():
    ...
    assert np.allclose(
        (X * 10).full(), 10 * X_full
    ), "Incorrect right scalar multiplication"
...
def test_LowRankMatrix_multiplication():
    ...
    assert np.allclose(
        X.dot(v, dense_output=True), A @ (B @ (C @ (D @ v)))
    ...
    assert (X.dot(Y)).deepshape == (
        ...
        8,
        7,
    ), "Incorrect deepshape of matrix-matrix product"
    assert np.allclose(
        X.dot(Y, dense_output=True), A @ (B @ (C @ (D @ Y)))
    ), "Incorrect matrix-matrix multiplication"
    Y = LowRankMatrix(
        np.random.randn(8, 5),
        np.random.randn(5, 6),
        np.random.randn(6, 4),
...
def test_compress_no_change():
    ...
    assert (
        len(X_compressed._matrices) == 2
    ), "Compression happened when it shouldn't have"
    assert np.allclose(
        X.full(), X_compressed.full()
    ), "Matrix value changed after compression"
...
def test_compress_one_merge():
    ...
    assert (
        len(X_compressed._matrices) == 2
    ), "Incorrect number of matrices after compression"
    assert X_compressed._matrices[0].shape == (
        10,
        2,
    ), "Incorrect shape of merged matrix"
    assert np.allclose(
        X.full(), X_compressed.full()
    ), "Matrix value changed after compression"
...
def test_compress_long_chain():
    ...
    assert (
        len(X_compressed._matrices) == 2
    ), "Incorrect number of matrices for long chain"
...
def test_compress_bottleneck_middle():
    ...
    assert X_compressed._matrices[0].shape == (
        10,
        2,
    ), "Shape of the left product is incorrect"
    assert X_compressed._matrices[1].shape == (
        2,
        12,
    ), "Shape of the right product is incorrect"
    assert np.allclose(
        X.full(), X_compressed.full()
    ), "Matrix value changed after compression"
...
def test_compress_bottleneck_first():
    ...
    assert X_compressed._matrices[0].shape == (
        10,
        2,
    ), "Shape of the first matrix should be unchanged"
    assert X_compressed._matrices[1].shape == (
        2,
        8,
    ), "Shape of the right product is incorrect"
...
def test_compress_bottleneck_second_to_last():
    ...
    assert X_compressed._matrices[1].shape == (
        2,
        8,
    ), "Shape of the last matrix should be unchanged"
    assert np.allclose(
        X.full(), X_compressed.full()
    ), "Matrix value changed after compression"
...
def test_compress_bottleneck_last_returns_dense():
    ...
    assert not isinstance(
        X_compressed, LowRankMatrix
    ), "Should not return a LowRankMatrix instance"
...
def test_complex_matrices():
    ...
    # Test dtype
    assert (
        X.dtype == np.complex128 or X.dtype == np.complex64
    ), "Incorrect dtype for complex matrix"
    ...
    assert np.allclose(
        (2 * X).full(), 2 * X_full
    ), "Complex scalar multiplication incorrect"
    assert np.allclose(
        ((1 + 1j) * X).full(), (1 + 1j) * X_full
    ), "Complex scalar multiplication incorrect"
...
def test_from_matrix():
    ...
    with pytest.raises(
        NotImplementedError, match="from_matrix\\(\\) must be implemented by subclasses"
    ):
...
def test_matmul_operator():
    ...
    result4 = X.dot(Z)
    assert np.allclose(
        result3.full(), result4.full()
    ), "@ operator should match dot() for LowRankMatrix"
...
def test_multiplication_operators():
    ...
    result1 = 3 * X_copy
    assert isinstance(
        result1, LowRankMatrix
    ), "Scalar * LowRankMatrix should return LowRankMatrix"
    assert np.allclose(
        result1.full(), 3 * X_full
    ), "Left scalar multiplication incorrect"

    result2 = X_copy * 3
    assert np.allclose(
        result2.full(), 3 * X_full
    ), "Right scalar multiplication incorrect"

    # Test in-place scalar multiplication
    X_copy2 = X.copy()
    X_copy2 *= 3
    assert isinstance(
        X_copy2, LowRankMatrix
    ), "In-place scalar multiplication should preserve type"
    assert np.allclose(
        X_copy2.full(), 3 * X_full
    ), "In-place scalar multiplication incorrect"
    ...
    assert isinstance(
        result3, np.ndarray
    ), "Hadamard with ndarray should return ndarray"
    assert np.allclose(
        result3, X_full * Y_dense
    ), "Hadamard multiplication with ndarray incorrect"
    ...
    assert isinstance(
        result4, np.ndarray
    ), "Hadamard with LowRankMatrix should return ndarray"
    assert np.allclose(
        result4, X_full * Y.full()
    ), "Hadamard multiplication with LowRankMatrix incorrect"
...
def test_dtype_property():
    ...
    ), "dtype incorrect for real matrix"
    ...
    assert (
        X_complex.dtype == np.complex128 or X_complex.dtype == np.complex64
    ), "dtype incorrect for complex matrix"
...
def test_dot_side_left():
    ...
    assert np.allclose(
        result2.full(), expected2
    ), "dot() with side='left' and LowRankMatrix incorrect"
...
def test_is_symmetric():
    ...
    rank = 4
    X_perturbed = LowRankMatrix(
        U[:, :rank] * np.sqrt(s[:rank]), np.sqrt(s[:rank])[:, None] * Vt[:rank, :]
    )
...
def test_dot_sparse():
    ...
    ), "Sparse left multiplication incorrect"

    # Test low-rank output
    result_lr = X.dot_sparse(sparse_csc, side="right", dense_output=False)
    assert isinstance(
        result_lr, LowRankMatrix
    ), "Should return LowRankMatrix when dense_output=False"
    assert np.allclose(
        result_lr.full(), expected
    ), "Sparse multiplication low-rank output incorrect"
    ...
    assert np.allclose(
        result_empty, np.zeros((10, 7))
    ), "Empty sparse matrix multiplication incorrect"
...
def test_expm_multiply():
    ...
    expected_left = sp.linalg.expm_multiply(
        A_sparse, X_small_full, start=0, stop=h, num=2, endpoint=True
    )[-1]
    ...
    result_right = X_small_right.expm_multiply(
        A_sparse, h, side="right", dense_output=True
    )
    expected_right = sp.linalg.expm_multiply(
        A_sparse.T, X_small_right_full.T, start=0, stop=h, num=2, endpoint=True
    )[-1].T
    assert np.allclose(
        result_right, expected_right
    ), "expm_multiply right side incorrect"

    # Test low-rank output
    result_lr = X_small.expm_multiply(A_sparse, h, side="left", dense_output=False)
    assert isinstance(
        result_lr, LowRankMatrix
    ), "expm_multiply should return LowRankMatrix when dense_output=False"
    assert np.allclose(
        result_lr.full(), expected_left
    ), "expm_multiply low-rank output incorrect"
...
def test_multi_dot():
    ...
    assert np.allclose(
        result2.full(), expected2
    ), "multi_dot with LowRankMatrix incorrect"

    # Test with empty sequence
    result_empty = X.multi_dot([])
    assert np.allclose(
        result_empty.full(), X_full
    ), "multi_dot with empty list should return copy"

    # Test with single matrix
    result_single = X.multi_dot([Y1])
    expected_single = X_full @ Y1
    assert np.allclose(
        result_single.full(), expected_single
    ), "multi_dot with single matrix incorrect"
...
def test_multi_add():
    ...
    assert isinstance(
        result_empty, np.ndarray
    ), "multi_add should always return ndarray"
    assert np.allclose(
        result_empty, X_full
    ), "multi_add with empty list should return full matrix"

    # Test with single matrix
    result_single = X.multi_add([Y1])
    expected_single = X_full + Y1.full()
    assert np.allclose(
        result_single, expected_single
    ), "multi_add with single matrix incorrect"
...
def test_extra_data_propagation():
    ...
    X_H = X_with_data.H
    assert (
        X_H._extra_data == metadata
    ), "extra_data not preserved in Hermitian transpose"

    # Test propagation through scalar multiplication
    X_scaled = 2 * X_with_data
    assert (
        X_scaled._extra_data == metadata
    ), "extra_data not preserved in scalar multiplication"
    ...
    assert (
        X_dot_lr._extra_data == metadata
    ), "extra_data not preserved in dot with LowRankMatrix"

    # Test propagation through compress
    X_compressed = X_with_data.compress()
    if isinstance(X_compressed, LowRankMatrix):
        assert (
            X_compressed._extra_data == metadata
        ), "extra_data not preserved in compress"
...
def test_create_matrix_alias():
    ...
    assert np.allclose(
        X_test.W_T, C.T
    ), "create_matrix_alias transpose getter incorrect"

    # Test setter
    new_A = np.random.randn(10, 5)
    X_test.U = new_A
    assert np.allclose(
        X_test._matrices[0], new_A
    ), "create_matrix_alias setter incorrect"
    assert np.allclose(
        X_test.U, new_A
    ), "create_matrix_alias setter/getter roundtrip incorrect"

    # Test transpose setter
    new_C = np.random.randn(4, 6)
    X_test.W_T = new_C
    assert np.allclose(
        X_test._matrices[2], new_C.T
    ), "create_matrix_alias transpose setter incorrect"
    assert np.allclose(
        X_test.W_T, new_C
    ), "create_matrix_alias transpose setter/getter roundtrip incorrect"
...
def test_create_matrix_alias_conjugate():
    ...
    assert np.allclose(
        X_complex._matrices[0], new_A.conj()
    ), "conjugate setter incorrect"

    # Test Hermitian setter
    new_A2 = np.random.randn(5, 10) + 1j * np.random.randn(5, 10)
    X_complex.U_H = new_A2
    assert np.allclose(
        X_complex._matrices[0], new_A2.T.conj()
    ), "Hermitian setter incorrect"
    ...
    try:
        ...
        for i in range(3):
            for j in range(3):
                ...
                assert (
                    abs(result[i, j] - expected_val) < 1e-10
...
def test_gather_multiple_elements():
    ...
    assert np.allclose(
        gathered_values, expected_values
...
def test_inefficiency_warning():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        assert any(
            issubclass(warning.category, LowRankEfficiencyWarning) for warning in w
        ), "Wrong warning type"
        # Should contain either "In-place" or "Hadamard product" in messages
        messages = [str(warning.message) for warning in w]
        assert any(
            "In-place" in msg or "Hadamard product" in msg for msg in messages
        ), "Expected Hadamard-related warning"
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        assert issubclass(
            w[-1].category, LowRankEfficiencyWarning
        ), "Wrong warning type"
        assert "Hadamard product" in str(
            w[-1].message
        ), "Wrong warning message for Hadamard"
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        assert "Subtraction" in str(
            w[-1].message
        ), "Wrong warning message for subtraction"
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        assert "symmetry" in str(
            w[-1].message
        ), "Wrong warning message for is_symmetric"
...
def test_addition_right_side():
    ...
    assert np.allclose(
        result_lr_left, result_lr_right
    ), "LowRankMatrix addition should be commutative"
...
def test_subtraction_left_right():
    ...
    assert not np.allclose(result_right, result_left) or np.allclose(
        result_left, 0
    ...
    assert np.allclose(
        result_lr_left, expected_lr_left
    ), "LowRankMatrix - LowRankMatrix incorrect"
    assert np.allclose(
        result_lr_right, expected_lr_right
    ), "LowRankMatrix - LowRankMatrix (reversed) incorrect"
    assert np.allclose(
        result_lr_left, -result_lr_right
    ), "LowRankMatrix subtraction non-commutativity failed"
...
def test_norm_edge_cases():
    ...
    assert norm_empty == 0 or np.isnan(
        norm_empty
    ), "Norm of empty matrix should be 0 or NaN"
...
def test_compress_equal_ranks():
    ...
    assert np.allclose(
        X_eq.full(), X_compressed.full()
    ), "Compression should preserve values"
...
def test_deepshape_unusual_chains():
    ...
    assert X_long.deepshape == (10,) + (3,) * 11 + (
        8,
    ), "Deepshape incorrect for long chain"
...
def test_sparse_side_aliases():
    ...
    assert np.allclose(
        result_usual, result_right
    ), "side='usual' should match side='right' for sparse"
    ...
    assert np.allclose(
        result_opposite, result_left
    ), "side='opposite' should match side='left' for sparse"
...
def test_diag_edge_cases():
    ...
    assert np.allclose(
        diag_small, np.diag(A_small @ B_small)
    ), "Small matrix diagonal incorrect"
...
def test_norm_squared_basic():
    ...
    assert (
        np.abs(norm_sq - norm_from_method) < 1e-8
    ), "Norm squared inconsistent with norm()"
...
def test_norm_squared_edge_cases():
    ...
    assert (
        np.abs(X_single.norm_squared() - 36.0) < 1e-10
    ), "Single element norm squared incorrect"
...
def test_compression_ratio():
    ...
    assert (
        np.abs(ratio - expected_ratio) < 1e-10
    ), "Compression ratio calculation incorrect"
...
def test_is_memory_efficient():
    ...
    X_good = LowRankMatrix(A_good, B_good)
    assert (
        X_good.is_memory_efficient
    ), "Low-rank matrix with rank 10 for 1000x1000 should be memory efficient"
    ...
    assert (
        not X_bad.is_memory_efficient
    ), "High-rank matrix (90 for 100x100) should not be memory efficient"
    ...
    X_empty = LowRankMatrix(A_empty, B_empty)
    assert (
        X_empty.is_memory_efficient
    ), "Empty matrix should be considered memory efficient"
...
def test_save_load_with_extra_data():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert np.allclose(
            X_loaded.full(), X_extra.full()
        ), "Matrix with extra data incorrect"
        assert "poles" in X_loaded._extra_data, "Extra data not loaded"
        assert np.allclose(
            X_loaded._extra_data["poles"], extra["poles"]
...
def test_to_sparse_threshold():
    ...
    with warnings.catch_warnings(record=True):
        ...
        assert (
            X_sparse.nnz < X_sparse.shape[0] * X_sparse.shape[1]
        ), "Threshold should reduce nnz"
...
def test_allclose():
    ...
    assert X.allclose(
        perturbed, rtol=1e-5, atol=1e-6
    ), "Should be close to small perturbation"

    # Large perturbation
    perturbed_large = X_full + 0.1 * np.random.randn(*X_full.shape)
    assert not X.allclose(
        perturbed_large, rtol=1e-5, atol=1e-6
    ), "Should not be close to large perturbation"
...
def test_new_features_complex():
    ...
    # Diagonal
    diag_c = X_c.diag()
    assert diag_c.dtype in [
        np.complex64,
        np.complex128,
    ], "Complex diagonal dtype incorrect"
    ...
    assert (
        np.abs(norm_sq_c - expected_norm_sq) / expected_norm_sq < 1e-6
    ), "Complex norm squared incorrect"
...
def test_edge_case_empty_dimensions():
    ...
    assert ratio >= 0 or np.isnan(
        ratio
    ), "Empty matrix ratio should be non-negative or NaN"
...
def test_linearoperator_addition():
    ...
    assert not isinstance(
        result, LowRankMatrix
    ), "Should return lazy sum, not LowRankMatrix"
    ...
    assert np.allclose(
        result_matvec, expected
    ), "LinearOperator addition matvec incorrect"

    # Reverse addition should also work
    result_rev = A_linop + X
    result_rev_matvec = result_rev @ v
    assert np.allclose(
        result_rev_matvec, expected
    ), "Reverse LinearOperator addition incorrect"
