...
def test_QR_from_matrix_transposed():
    ...
    assert np.allclose(X_trans.full(), A_test.T.conj()), "from_matrix transposed reconstruction incorrect"
...
def test_QR_Q_R_properties():
    ...
    assert np.allclose(X_std.full(), A_test), "Reconstruction incorrect in standard mode"
    ...
    assert np.allclose(X_trans.full(), A_test.T.conj()), "Reconstruction incorrect in transposed mode"
...
def test_QR_factors_consistency():
    ...
    assert np.allclose(X_trans.full(), R.T.conj() @ Q.T.conj()), "Transposed mode reconstruction"
    assert np.allclose(X_std.full(), A), "Standard mode should reconstruct A"
    assert np.allclose(X_trans.full(), A.T.conj()), "Transposed mode should reconstruct A.H"
...
def test_QR_transpose_conjugate_hermitian():
    ...
    assert np.allclose(X_real.H.full(), A_real.T), "Real: .H should give A.T (same as .T)"
    assert np.allclose(X_real.conj.full(), A_real), "Real: .conj should give A (no change)"
    ...
    assert np.allclose(X_real_trans.full(), A_real.T), "Real transposed mode should give A.T"
    assert np.allclose(X_real_trans.T.full(), A_real.conj()), "Real transposed: .T of A.H gives conj(A) (= A for real)"
    assert np.allclose(X_real_trans.H.full(), A_real), "Real transposed: .H of A.H gives A"
    ...
    assert np.allclose(X_complex.H.full(), A_complex.T.conj()), "Complex: .H should give A.H"
    assert np.allclose(X_complex.conj.full(), A_complex.conj()), "Complex: .conj should give conj(A)"
    ...
    assert np.allclose(X_complex_trans.full(), A_complex.T.conj()), "Complex transposed mode should give A.H"
    assert np.allclose(X_complex_trans.T.full(), A_complex.conj()), "Complex transposed: .T of A.H gives conj(A)"
    assert np.allclose(X_complex_trans.H.full(), A_complex), "Complex transposed: .H of A.H gives A"
    ...
    assert np.allclose(X_complex.conj.conj.full(), A_complex), "Complex: .conj.conj should give A"
...
def test_QR_is_orthogonal():
    ...
    assert X_proper.is_orthogonal(tol=1e-12), "QR should be orthogonal with tight tolerance"
    ...
    assert not X_bad.is_orthogonal(tol=1e-1), "Random Q should not be orthogonal even with loose tolerance"
...
def test_QR_is_upper_triangular():
    ...
    A = np.random.randn(100, 50)
    X_proper = QR.from_matrix(A)
    assert X_proper.is_upper_triangular(), "QR from from_matrix should be upper triangular"
    assert X_proper.is_upper_triangular(tol=1e-12), "R should be upper triangular with tight tolerance"
    
    # Test with transposed mode
    X_trans = QR.from_matrix(A, transposed=True)
    assert X_trans.is_upper_triangular(), "QR in transposed mode should have upper triangular R"
    
    # Test with generated random QR
    X_random = QR.generate_random((80, 60), seed=42)
    assert X_random.is_upper_triangular(), "Generated random QR should have upper triangular R"
    ...
    assert X_almost.is_upper_triangular(tol=1e-12), "Should be upper triangular with appropriate tolerance"
    assert not X_almost.is_upper_triangular(tol=1e-15), "Should fail with very tight tolerance"
...
def test_QR_is_upper_triangular_complex():
    ...
    Y = QR(Q, R_bad)
    assert not Y.is_upper_triangular(), "Random complex R should not be upper triangular"
...
def test_QR_scalar_multiplication():
    ...
    assert np.allclose(Y_mul.full(), scalar * A), "Multiplication should give scalar * A"
    ...
    # Test __rmul__
    Y_rmul = scalar * X_std
    assert np.allclose(Y_rmul.full(), scalar * A), "Right multiplication should give scalar * A"
    ...
    X_imul *= scalar
    assert np.allclose(X_imul.full(), scalar * A), "In-place multiplication should give scalar * A"
    ...
    assert np.allclose(Y_trans.full(), scalar * A.T.conj()), "Transposed mode scalar multiplication"
...
def test_QR_hadamard_product():
    ...
    assert np.allclose(Z_method.full(), A * B), "Hadamard product should match element-wise multiplication"
    ...
    assert np.allclose(Z_op.full(), A * B), "* operator should give same result as hadamard"
    
    # Test with ndarray using * operator
    Z_array = X * B
    assert isinstance(Z_array, np.ndarray), "QR * ndarray should return ndarray"
    assert np.allclose(Z_array, A * B), "QR * ndarray should match element-wise multiplication"
    
    # Test ndarray * QR (reverse)
    Z_array_rev = B * X
    assert isinstance(Z_array_rev, np.ndarray), "ndarray * QR should return ndarray"
    assert np.allclose(Z_array_rev, A * B), "ndarray * QR should match element-wise multiplication"
    
    # Test with hadamard method and ndarray
    Z_method_array = X.hadamard(B)
    assert isinstance(Z_method_array, np.ndarray), "hadamard(ndarray) should return ndarray"
    assert np.allclose(Z_method_array, A * B), "hadamard(ndarray) should match element-wise multiplication"
...
def test_QR_hadamard_complex():
    ...
    assert np.allclose(Z.full(), A_complex * B_complex), "Complex Hadamard should work correctly"
...
def test_QR_hadamard_efficiency_warning():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        assert any("Hadamard product creates Q with" in msg for msg in warning_messages), \
            "Should warn about Q dimensions"
    
    # But the result should still be correct
    assert np.allclose(Z.full(), A * B), "Hadamard should still be correct despite warning"
...
def test_QR_addition_mismatched_transposed():
    ...
    assert isinstance(Z, np.ndarray), "Addition with mismatched flags should return ndarray"
    assert not isinstance(Z, QR), "Addition with mismatched flags should not return QR"
    # But result should still be correct
    assert np.allclose(Z, A + B.T), "Addition should be correct even with mismatched flags"
...
def test_QR_subtraction_mismatched_transposed():
    ...
    assert isinstance(Z, np.ndarray), "Subtraction with mismatched flags should return ndarray"
    assert not isinstance(Z, QR), "Subtraction with mismatched flags should not return QR"
...
def test_QR_dot_transposed_mode():
    ...
    assert np.allclose(D.full(), D_left @ A.T), "ndarray @ transposed QR should be correct"
...
def test_QR_truncate_reconstruction_quality():
    ...
    assert reconstruction_error < 10 * noise_level, "Truncation should capture dominant structure"
...
def test_QR_truncate_both_modes():
    ...
    assert np.allclose(X_trunc.full(), expected), "Standard mode truncation reconstruction"
    ...
    assert np.allclose(Z_trunc.full(), expected_Z), "Transposed mode truncation reconstruction"
...
def test_QR_solve_tall_matrix():
    ...
    assert np.allclose(A @ x, b, atol=1e-10), "Should solve exactly for consistent system"
...
def test_QR_solve_complex_dtype():
    ...
    np.random.seed(705)
    A_complex = np.random.randn(60, 60) + 1j * np.random.randn(60, 60)  # Square for exact solution
    ...
    assert np.allclose(A_complex @ x, b_complex, atol=1e-10), "Complex solve should work"
...
def test_QR_lstsq_basic():
    ...
    assert np.abs(residual - residual_numpy) < 1e-9, "Should achieve similar residual as numpy"
...
def test_QR_lstsq_multiple_rhs():
    ...
    for i in range(5):
        ...
        assert np.abs(residual - residual_numpy) < 1e-8, f"Column {i} should match numpy"
...
def test_QR_pseudoinverse_complex():
    ...
    assert np.allclose(A_complex @ A_pinv @ A_complex, A_complex, atol=1e-9), "Property 1"
...
def test_QR_pseudoinverse_transposed_mode():
    ...
    A_T = A.T
    assert np.allclose(A_T @ A_pinv @ A_T, A_T, atol=1e-9), "Property 1 in transposed mode"
...
def test_QR_pseudoinverse_solve_consistency():
    ...
    assert np.allclose(x_solve, x_pinv, atol=1e-9), "solve and pseudoinverse should agree"
...
def test_QR_pseudoinverse_consistency_with_numpy():
    ...
    assert np.allclose(A_pinv_qr, A_pinv_numpy, atol=1e-9), "Should match numpy.linalg.pinv"
...
def test_QR_pseudoinverse_idempotent():
    ...
    AA_pinv = A @ A_pinv
    assert np.allclose(AA_pinv @ AA_pinv, AA_pinv, atol=1e-9), "(A @ A+) should be idempotent"
    
    # (A+ @ A) should be idempotent: (A+ @ A)^2 = A+ @ A
    A_pinvA = A_pinv @ A
    assert np.allclose(A_pinvA @ A_pinvA, A_pinvA, atol=1e-9), "(A+ @ A) should be idempotent"
...
def test_QR_cond_singular():
    ...
    A = np.array([[1, 0, 0],
                   [0, 1, 0],
                   [0, 0, 1e-20]])  # Essentially singular
...
def test_QR_to_svd_basic():
    ...
    assert np.allclose(X_qr.full(), X_svd.full(), atol=1e-10), "Should reconstruct same matrix"
...
def test_QR_from_svd_basic():
    ...
    # Check reconstruction
    assert np.allclose(X_svd.full(), X_qr.full(), atol=1e-10), "Should reconstruct same matrix"
...
def test_QR_svd_roundtrip():
    ...
    assert np.allclose(X_qr1.full(), X_qr2.full(), atol=1e-10), "Roundtrip should preserve matrix"
...
def test_QR_svd_roundtrip_reverse():
    ...
    # Check reconstruction after roundtrip
    assert np.allclose(X_svd1.full(), X_svd2.full(), atol=1e-10), "Roundtrip should preserve matrix"
    
    # Singular values might differ in order/sign, but matrix should be same
    assert np.allclose(np.sort(X_svd1.s)[::-1], np.sort(X_svd2.s)[::-1], rtol=1e-10), \
...
def test_QR_to_svd_transposed():
    ...
    assert np.allclose(X_qr_trans.full(), X_svd_trans.full(), atol=1e-10), "Should represent same matrix"
...
def test_QR_to_svd_orthogonality():
    ...
    U_orthogonal = np.allclose(X_svd.U.T.conj() @ X_svd.U, np.eye(X_svd.rank), atol=1e-10)
    assert U_orthogonal, "U should have orthonormal columns"
    
    # Check V has orthonormal columns
    V_orthogonal = np.allclose(X_svd.V.T.conj() @ X_svd.V, np.eye(X_svd.rank), atol=1e-10)
