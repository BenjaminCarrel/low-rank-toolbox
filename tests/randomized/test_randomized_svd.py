...
from lowrank.randomized import randomized_svd, adaptive_randomized_svd
...
class TestRandomizedSVD:
    ...
    def test_orthogonality_u(self, low_rank_matrix):
        ...
        identity = np.eye(r)
        assert_allclose(result.U.T @ result.U, identity, atol=1e-10,
    ...
    def test_orthogonality_v(self, low_rank_matrix):
        ...
        identity = np.eye(r)
        assert_allclose(result.V.T @ result.V, identity, atol=1e-10,
    ...
    def test_singular_values_close_to_true(self, low_rank_matrix):
        ...
        assert_allclose(result.s[:r], s, rtol=1e-6, atol=1e-10,
    ...
    def test_large_matrix_oversampling_sweep(self, large_low_rank_matrix):
        ...
        assert errors[-1] <= errors[0] * (1 + 1e-10), \
    ...
    def test_invalid_oversampling(self, low_rank_matrix):
        ...
        with pytest.raises(ValueError, match="Oversampling parameter must be non-negative"):
    ...
    def test_invalid_power_iterations(self, low_rank_matrix):
        ...
        with pytest.raises(ValueError, match="Number of power iterations must be non-negative"):
...
class TestAdaptiveRandomizedSVD:
    ...
    def test_tighter_tolerance_increases_rank(self, low_rank_matrix):
        ...
        result1 = adaptive_randomized_svd(A_noisy, tol=1e-3, failure_prob=1e-6, seed=1234)
        result2 = adaptive_randomized_svd(A_noisy, tol=1e-5, failure_prob=1e-6, seed=1234)
        
        assert result2.rank >= result1.rank, \
            "Tighter tolerance should increase rank"
    ...
    def test_max_rank_constraint(self, low_rank_matrix):
        ...
        result = adaptive_randomized_svd(A, tol=1e-10, failure_prob=1e-6, 
                                        max_rank=max_r, seed=1234)
        
        assert result.rank <= max_r, \
            f"Rank {result.rank} exceeds max_rank {max_r}"
