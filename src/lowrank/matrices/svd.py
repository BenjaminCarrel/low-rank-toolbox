...
class QuasiSVD(LowRankMatrix):
    ...
    def to_svd(self) -> SVD:
        ...
        output.U = self.U.dot(output.U)
        output.V = self.V.dot(output.V)
        return output
    
    def truncate(self, r: int = None, rtol: float = default_atol, atol: float = default_atol) -> SVD:
    ...
    def project_onto_DEIM_tangent_space(self, Y_u: ndarray, Y_v: ndarray, Y_uv: ndarray, M_u: ndarray, M_v: ndarray, truncate: bool = automatic_truncation) -> SVD:
        ...
        M_u = U (P_U^* U)^{-1}, M_v = U (P_V^* V)^{-1}, 
        Y_u = Y[p_u, :], Y_v = Y[:, p_v] and Y_uv = Y[p_u, p_v] where p_u and p_u are the DEIM indices of U and V.
