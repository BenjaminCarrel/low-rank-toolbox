    ...
    **Memory efficiency:**
    ...
    def T(self) -> 'QR':
        ...
        if self._transposed:
            ...
            return QR(self.Q.conj(), self.R.conj(), transposed=False, **self._extra_data)
        else:
    ...
    def conj(self) -> 'QR':
        ...
        """
        return QR(self.Q.conj(), self.R.conj(), transposed=self._transposed, **self._extra_data)
    ...
    def is_upper_triangular(self, tol: float = 1e-12) -> bool:
        ...
        for i in range(1, r):  # Start from row 1 (skip first row which has no elements below)
    ...
    def norm(self, ord: str | int = 'fro') -> float:
        ...
        if ord == 'fro':
            result = np.linalg.norm(self.R, 'fro')
        else:
            # Other norms require full matrix
            result = np.linalg.norm(self.full(), ord)
    ...
    def __add__(self, other: QR | LowRankMatrix | ndarray) -> Union[QR, ndarray]:
        ...
        **With mismatched modes:**
        ...
        if isinstance(other, QR):
            _condition = (self._transposed and other._transposed) or (not self._transposed and not other._transposed)
            if _condition:
    ...
    def __mul__(self, other: float | int | QR) -> QR:
        """
    ...
    def __rmul__(self, other: float | int | QR) -> QR:
        """
    ...
    def __imul__(self, other: float | int | QR) -> QR:
        """
        ...
        """
        if isinstance(other, QR):
            warnings.warn("In-place multiplication between two QR is not implemented, falling back to regular multiplication (no memory saved).", UserWarning)
            return self.__mul__(other)
        elif isinstance(other, (float, int, np.number)):
    ...
    def hadamard(self, other: QR | ndarray) -> QR | ndarray:
        ...
        """
        if isinstance(other, QR):
            condition = (self._transposed and other._transposed) or (not self._transposed and not other._transposed)
            if not condition:
                raise ValueError("Hadamard product is only implemented between two QR matrices in the same mode (both transposed or both standard).")
            Q_new = np.zeros((self.Q.shape[0], self.Q.shape[1] * other.Q.shape[1]), dtype=self.Q.dtype)
            R_new = np.zeros((self.R.shape[0] * other.R.shape[0], self.R.shape[1]), dtype=self.R.dtype)
    ...
    ## CLASS METHODS
    @classmethod
    def from_matrix(cls, matrix: ndarray, mode: str = 'economic', transposed: bool = False, **extra_data):
    ...
    @classmethod
    def from_low_rank(cls, low_rank: LowRankMatrix, transposed: bool = False, **extra_data):
    ...
    @classmethod
    def qr(cls, matrix: ndarray, mode: str = 'economic', transposed: bool = False, **extra_data) -> QR:
    ...
    @classmethod
    def generate_random(cls, shape: tuple, transposed: bool = False, seed: int = None, **extra_data) -> QR:
    ...
    def truncate(self, r: int = None, rtol: float = DEFAULT_RTOL, atol: float = DEFAULT_ATOL, 
             inplace: bool = False) -> 'QR':
    ...
    def solve(self, b: ndarray, method: str = 'direct') -> ndarray:
        ...
        if b.shape[0] != self.shape[0]:
            raise ValueError(f"Dimension mismatch: matrix has {self.shape[0]} rows but b has {b.shape[0]} rows")
    ...
    def lstsq(self, b: ndarray, rcond: float = None) -> ndarray:
    ...
    def pseudoinverse(self, rcond: float = None) -> ndarray:
    ...
    def cond(self, p: int | str = 2, exact: bool = False) -> float:
        ...
        **Why use approximation?**
        ...
        if p == 2 and not exact:
            ...
            result = np.linalg.cond(self.R, p)
